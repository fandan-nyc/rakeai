...
\renewcommand{\headrulewidth}{0pt}
    ...
    doc.preamble.append(Command("setlength", arguments=NoEscape(r"\headheight"), options={}, extra_arguments="2cm"))
    ...
    with doc.create(FlushLeft()):
        doc.append(NoEscape(r"""Table 1. The representative structure of a series of lipid structures predicted by LNPCompassTM. The
    ...
    with doc.create(Center()):
        doc.append(NoEscape(r"\renewcommand{\arraystretch}{2}"))
        doc.append(NoEscape(r"\setlength{\tabcolsep}{5.2mm}{")) # remember add a } in the end
        with doc.create(LongTable(table_spec=r"|>{\centering\arraybackslash}p{5mm}|>{\centering\arraybackslash}p{1cm}|>{\centering\arraybackslash}p{6.5cm}|>{\centering\arraybackslash}p{2cm}|")) as table:
            table.add_hline() 
            table.add_row([NoEscape("Top"),"Name","Structure","Probability of Effectiveness"])
            doc.append(NoEscape(r"\endfirsthead"))
            table.add_hline()
            table.add_row(["Top","Name","Structure","Probability of Effectiveness"])
            ...
            for index, d in enumerate(data.data):
                ...
                line_data[2] = NoEscape(r"\raisebox{-0.1cm}[2.5cm]{\includegraphics[height=2.5cm]{"+pic_dir+r"}}")
